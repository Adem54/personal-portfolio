...
if(width<=720){
  ...
  return (
    <div className=" bg-black/[.06] fixed inset-0 z-50 "
    onClick={()=> {
        console.log("Clicked")
        if(showModal){
          setShowModal(false)
    ...
    <div className="flex h-screen justify-center items-center px-4  ">
        ...
        <div className="flex-col justify-center xl:w-1/3 lg:w-1/2 md:w-3/4 sm:w-3/4 w-1/1  h-screen-auto relative z-60   bg-red-300   border-4 border-sky-500 rounded-xl "
        onClick={(e)=>{
            e.stopPropagation();
        }}
        >
            ...
            <div className="flex px-4 py-4 mb-6 flex-col items-center">
             ...
             <a href={githubUrl} className="bg-red-500 px-7 py-2 rounded-md text-xl mt-4 text-white" >Go Github</a>
            </div>
            <div  className="absolute bottom-6 right-6 text-3xl cursor-pointer z-60"
